      ...
      <div className="lg:ml-64">
        <header className="sticky top-0 z-30 flex h-20 items-center gap-3 border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90 sm:px-6 lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-3">
            ...
            <button aria-label="Open account settings" className="flex items-center gap-2 rounded-xl px-2 py-1.5 text-left hover:bg-slate-100 dark:hover:bg-slate-900" onClick={() => navigate("/settings")} type="button"><UserCircle className="size-8 text-slate-500 dark:text-slate-400" strokeWidth={1.5} /><span className="hidden sm:block"><span className="block text-sm font-semibold text-slate-800 dark:text-slate-100">{user?.name}</span><span className="block text-xs text-slate-500 dark:text-slate-400">{primaryRole}</span></span></button>
